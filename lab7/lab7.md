### 2019/7/19, lab 7. Robin Hong, hongz@rpi.edu.
...
The modified code `step1_plot_words.py` from `plot_words.py` is:
...
The modified code `step3_plot_words.py` from `plot_words.py` is:
...
if __name__ == '__main__':
    G = words_graph()
    print("Loaded words_dat.txt containing %d five-letter English words." % (nx.number_of_nodes(G)))
    print("Two words are connected if they differ in one letter.")
    print("Graph has %d nodes with %d edges"
          % (nx.number_of_nodes(G), nx.number_of_edges(G)))
    print("%d connected components" % nx.number_connected_components(G))
...
The modified code `step4_plot_words.py` from `step3_plot_words.py` is:
...
def generate_graph(words):
    ...
    def edit_distance_one(word):
        for i in range(len(word)):
            ...
            j = lookup[c]  # lowercase.index(c)
            for cc in lowercase[j + 1:]:
                new_string = left + cc + right
                for x in itertools.permutations(new_string):
                    yield ''.join(x)
    candgen = ((word, cand) for word in sorted(words)
               for cand in edit_distance_one(word) if cand in words)
    G.add_nodes_from(words)
